...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="austenWork in austen.famousWorks" :key="austenWork">
          {{ austenWork.title }} ({{ austenWork.year }})
        </li>
      </ul>
    </section>
    ...
    <section class="lab-section">
      ...
      <!-- Task 2 highlight authors-->
      <fieldset>
        <legend>Specific Authors:</legend>
        <div v-for="author in authors" :key="author.id">
          <label
            :for="author.id"
            :class="{ highlight: selectedAuthor === author.id }"
            :style="highlightAuthor(author.id)"
          >
            <input
              type="radio"
              :id="author.id"
...
const allFamousWorks = computed(() => {
  ...
  // reduce iterate over each element inside authors array
  // return authors.reduce((works,author) => {return works.concat(author.famousWorks)},[])
})
...
// Assign selected radio button value to selectedAuthor
const selectAuthor = (id) => {
  selectedAuthor.value = id
}
// selected author label in bold
const highlightAuthor = (id) => {
  return {
    fontWeight: selectedAuthor.value === id ? 'bold' : 'normal'
  }
}
</script>
